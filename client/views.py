...
@permission_required('client.add_clientgroup', login_url='login')
def manage_client_group(request, pk=None):
    ...
    return render(request, 'client/create_edit_group.html', context)


@login_required
def upload_client_groups(request):
    if request.method == "POST":
        ...
        for row in csv.reader(io_string, delimiter=','):
            ...
            client_code = row[0].strip()
            group_name = row[1].strip()

            try:
                group_name = group_name.title()
                group, _ = ClientGroup.objects.get_or_create(name=group_name)

                updated = Client.objects.filter(
                    internal_id_number=client_code).update(client_group=group)
                if updated:
                    count += 1
            except Exception as e:
                continue

        messages.success(request, f"Successfully updated {count} clients.")
        return redirect(reverse("process"))

    return render(request, 'client/upload_groups.html')
